...
const fidanAssignmentExpressionSetCompute = (
  expression: t.AssignmentExpression,
  fComputeParameters: any[]
) => {
...
const expressionStatementGeneralProcess = (
  ...
  if (t.isAssignmentExpression(expression)) {
    ...
    if (t.isMemberExpression(expression.left) && !isExport) {
      ...
      if (
        leftIsTracked &&
        check.isClassPropertyLike(path, expression) === false
      ) {
        // class-property-1
        if (rightIsTracked) return;
          ...
          if (check.isDynamicExpression(expression.right)) {
            ...
            if (fComputeParameters.length > 0) {
              expression.right = dynamicExpressionInitComputeValues(
                expression.right,
                fComputeParameters
              );
              return;
      ...
      if (rightIsTracked) {
        if (leftIsTracked) {
          path.node[propertyName] = modify.fidanCall(
            expression.left,
            expression.right,
            expression.operator
          );
    ...
    if (
      check.isTrackedVariable(scope, expression.left) &&
      ...
      if (containsAnotherTracked) {
        expression.right = modify.fidanAssignmentExpressionSetCompute(
          expression,
          fComputeParameters
        );
      } else {
...
export const moveContextArguments = (args: any[], contextArgIndex: number) => {
  ...
  contextArgs.push(
    t.callExpression(
      t.memberExpression(t.identifier("fidan"), t.identifier("endContext")),
      [contextArgProps[0].value]
    )
  );
  args[contextArgIndex] = t.callExpression(
    t.memberExpression(t.identifier("fidan"), t.identifier("startContext")),
