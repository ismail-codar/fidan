...
export function setAttrExpr(elem, name, value) {
	return t.expressionStatement(
		t.callExpression(t.memberExpression(t.identifier(globalOptions.moduleName), t.identifier('wrap')), [
...
export function computeAttribute(elem, name, value) {
	...
	const node = t.callExpression(t.memberExpression(t.identifier(globalOptions.moduleName), t.identifier('attr')), [
		elem,
		t.stringLiteral(name),
...
export function createTemplate(path: any, results: GenerationResultType, isFragment?: boolean) {
	let decl;
...
export const insertOrConditional = (results: GenerationResultType, innerExpr, exprId) => {
	const methodName = t.isConditionalExpression(innerExpr) ? 'conditional' : 'insert';
	results.exprs.push(
		t.expressionStatement(
			t.callExpression(t.memberExpression(t.identifier(globalOptions.moduleName), t.identifier(methodName)), [
				results.id,
				t.isConditionalExpression(innerExpr)
					? t.objectExpression([
							t.objectProperty(
								t.stringLiteral('test'),
								t.isCallExpression(innerExpr.test) && innerExpr.test.arguments.length > 0
									? innerExpr.test // computed(..)
									: t.arrowFunctionExpression([], innerExpr.test)
							),
							t.objectProperty(
								t.identifier('consequent'),
								t.arrowFunctionExpression([], innerExpr.consequent)
							),
							t.objectProperty(
								t.identifier('alternate'),
								t.arrowFunctionExpression([], innerExpr.alternate)
							)
						])
					: canBeReactive(innerExpr) ? innerExpr.callee : innerExpr,
				t.nullLiteral(),
				exprId
			])
		)
...
export const arrayMapExpression = (results: GenerationResultType, innerExpr: t.CallExpression, exprId) => {
	results.exprs.push(
		t.expressionStatement(
			t.callExpression(t.memberExpression(t.identifier(globalOptions.moduleName), t.identifier('arrayMap')), [
				results.id,
