		...
		if (t.isJSXElement(jsx.children[i])) {
			...
			if (
				jsx.children[i].openingElement.attributes.some(
					(attr) => t.isJSXSpreadAttribute(attr) || t.isJSXExpressionContainer(attr.value)
				)
			)
				return true;
			if (jsx.children[i].children.length) if (detectExpressions(jsx.children[i], 0)) return true;
		}
	}
}

export const insertFidanImport = (body: t.Node[]) => {
	body.splice(
		0,
		0,
		t.variableDeclaration('var', [
			t.variableDeclarator(
				...
				t.callExpression(t.identifier('require'), [ t.stringLiteral('@fidanjs/jsx') ])
			)
		])
	);
};

export const isComponentNode = (id: t.Expression | t.V8IntrinsicIdentifier) => {
	let tagName = t.isIdentifier(id)
		? id.name
		: t.isMemberExpression(id) ? (t.isIdentifier(id.property) ? id.property.name : id.property) : null;
...
export const canBeReactive = (value: t.Expression): value is t.CallExpression => {
	const reactive = t.isCallExpression(value) && value.arguments.length == 0 && !isComponentNode(value.callee);
	return reactive;
};
...
export const jsxParentComponent = (
	...
	if (t.isVariableDeclarator(declarationNode)) {
		...
		if (
			...
			(t.isArrowFunctionExpression(declarationNode.init) || t.isFunctionExpression(declarationNode.init))
		) {
			if (declarationNode.init.params.length === 1 && t.isIdentifier(declarationNode.init.params[0])) {
				return {
					componentName,
	...
	if (t.isFunctionDeclaration(declarationNode)) {
		...
		if (isComponentName(componentName)) {
			if (declarationNode.params.length === 1 && t.isIdentifier(declarationNode.params[0])) {
				return {
					componentName,
